...
find examples -maxdepth 1 -name '*.cpp' -exec basename {} .cpp \; | sort >"$tmpdir/cpp.txt"
find examples -maxdepth 1 -name '*.rb' -exec basename {} .rb \; | sort >"$tmpdir/rb.txt"
...
find examples -maxdepth 1 -name '*.s' -exec basename {} .s \; | sort >"$tmpdir/s.txt"

go_count=$(wc -l <"$tmpdir/go.txt" | tr -d ' ')
js_count=$(wc -l <"$tmpdir/js.txt" | tr -d ' ')
c_count=$(wc -l <"$tmpdir/c.txt" | tr -d ' ')
cpp_count=$(wc -l <"$tmpdir/cpp.txt" | tr -d ' ')
rb_count=$(wc -l <"$tmpdir/rb.txt" | tr -d ' ')
...
if [[ $go_count != "$js_count" || $go_count != "$c_count" || $go_count != "$cpp_count" || $go_count != "$rb_count" || $go_count != "$sh_count" || $go_count != "$bat_count" || $go_count != "$ps1_count" || $go_count != "$s_count" ]]; then
  printf 'example count mismatch: go=%s js=%s c=%s cpp=%s rb=%s sh=%s bat=%s ps1=%s s=%s\n' "$go_count" "$js_count" "$c_count" "$cpp_count" "$rb_count" "$sh_count" "$bat_count" "$ps1_count" "$s_count" >&2
  exit 1
fi

for lang in js c cpp rb sh bat ps1 s; do
...
if [[ ! -f examples/greeter_cpp/index.cpp ]]; then
  printf 'examples/greeter_cpp/index.cpp is required\n' >&2
  find examples/greeter_cpp -maxdepth 1 -type f -print >&2
  exit 1
fi

if [[ ! -f examples/greeter_ruby/index.rb ]]; then
  printf 'examples/greeter_ruby/index.rb is required\n' >&2
  find examples/greeter_ruby -maxdepth 1 -type f -print >&2
  exit 1
fi
...
printf 'structure ok: %s examples per language for go/js/c/cpp/rb/sh/bat/ps1/s\n' "$go_count"
